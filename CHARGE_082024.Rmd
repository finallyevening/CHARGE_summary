...
#CHARGE
CHARGE_batches_reviewed<- read_excel('J:\\PM\\ORG-GC\\CHARGE\\CHARGE_Batches__reviewedBA020524.xlsx')



sheets_to_read <- readxl::excel_sheets("J:\\PM\\ORG-GC\\CHARGE\\CHARGE_Batches__reviewedBA020524.xlsx")

CHARGE_batches_reviewed <- lapply(1:length(sheets_to_read), function(i)readxl::read_excel("J:\\PM\\ORG-GC\\CHARGE\\CHARGE_Batches__reviewedBA020524.xlsx", sheet = sheets_to_read[i]) %>% mutate(tabname = sheets_to_read[i]))

as.numeric(CHARGE_batches_reviewed)

CHARGE_batches_reviewed %>%
  kbl( longtable = TRUE , 
...
data_trans <- CHARGE_batches_reviewed %>% 
...
```{r JH Drexel notes Example, echo=FALSE,message=FALSE,warning=FALSE}
all_notes_confirmed %>%
  mutate(`Tooth Powder`= all_notes_confirmed$`Tooth Powder`*22) %>% 
  select("#","Notes","Tooth Powder","Start Drill date","End Drill date","Date of Weighing", "Fraction") %>% 
  kbl(
    caption = "<center><span style='font-size:30px'>Raw notes</span></center>") %>% 
  kable_styling( stripe_color = "gray!6",
    bootstrap_options = c("striped", "hover")) %>% 
...
df2 %>%
  ...
  select('Tooth Powder','Fraction.x','Drill Round #','Extraction Batch','Extraction Date','Cleanup date','#','Number of samples (n) in the batch','Total # samples') %>% 
  kbl(
    caption = "<center><span style='font-size:30px'>Batches 1-63</span></center>") %>% 
  kable_styling( stripe_color = "gray!6",
    bootstrap_options = c("striped", "hover")) %>% 
    scroll_box(width = "700px", height = "500px") 
...
T_2_OCP %>%
     mutate(across(is.numeric,~ .* 0.23)) %>% 
  select(-'Lab sample ID',) %>% 
  select(-'Extraction Label') %>% 
    kbl(
...
T_3_OCP %>%
  mutate(across(is.numeric,~ .* 0.2123)) %>% 
  select(-'Lab sample ID',) %>% 
  select(-'Extraction Label') %>% 
      kbl(
...
T_Post_OCP %>%
  mutate(across(is.numeric,~ .* 0.555)) %>% 
  select(-'Lab sample ID',) %>% 
  select(-'Extraction Label') %>% 
    kbl(
...
#Spiked
OCPPBDE_QC_Spiked %>%
  mutate(across(is.numeric,~ .* 0.4213)) %>% 
    select(-'Data File',) %>% 
  select(-'Extraction Label') %>% 
  kbl(
...
ocp_batches %>%
  mutate(value=ocp_batches$value *0.32) %>%
...
pcb_batches %>%
  mutate(value=pcb_batches$value *0.32) %>%
  group_by(Stage, Batch_num) %>% 
  ggplot(aes(x = factor(Batch_num),  y = value,fill=Stage)) +
      labs(y="Concentration [ppb]", x = "Batch number", 
...
avg_plot1<- max_min_spiked_ocp %>%
  mutate(value=max_min_spiked_ocp$value *0.632) %>%
  ...
  geom_label_repel(data = ocp_hline, 
                  aes(x = 4, y = ocp_hline$mean + 0.01, label = paste0("x̄=", sprintf("%.3f",ocp_hline$mean))),#sig figs
...
avg_plot2<- max_min_spiked_pcb %>%
  mutate(value=max_min_spiked_pcb$value *0.732) %>%
  ggplot(aes(x = factor(Batch_num2), y = value)) + 
  facet_wrap(~variable, scales="free",ncol=1)+
    geom_blank(aes(y = y_min)) +
    geom_blank(aes(y = y_max))+
  ...
  geom_point(aes(color=`Batch_num`)) +
    geom_label_repel(data = pcb_hline, 
                  aes(x = 4, y = pcb_hline$mean + 0.01, label = paste0("x̄=", sprintf("%.3f",pcb_hline$mean))),#sig figs
                  size=3,point.size = NA, #Do not repel labels from data points
...
#```{r OCP QC SCatterplot - display plot, all samples, echo=FALSE,message=FALSE,warning=FALSE, fig.height = 30, fig.width =10 }
ocp_qc_batches %>%
    group_by(Type, Batch_num) %>% 
  ggplot(aes(x = factor(Batch_num),  y = value)) +
  geom_point(aes(color=`Batch_num`))+
  facet_wrap(~variable,scales = "free", ncol=1)+
      labs(y="Concentration [ppb]", x = "Batch number", 
       title =  paste0("Batch Effect Evaluation: PCB \nSample concentration across all batches (grouped by analyte) "),
...
```{r OCP Unspike plot: Running average distribution, echo=FALSE,message=FALSE,warning=FALSE,  fig.height = 60, fig.width =15}
...
shewhart_ocp_unspike<- max_min_unspike_ocp %>%
  ...
  theme(axis.title.y = element_text(size=10, face="bold", colour = "black"),
        axis.text.y = element_text(size=10, face="bold", colour = "black"),
        strip.text = element_text(face = "bold", size=10))+
  labs(y="Concentration", x = "Batch number")+
  ggtitle("QC Unspiked : OCP distribution")+
  annotate("text",x=10, y=0.001, size=4, label = paste("n=",max_min_unspike_ocp$`n (per analyte)`[1] ))
...
shewhart_pcb_unspike<-ggplot(max_min_unspike_pcb, aes(x = factor(Batch_num2), y = value, color=Batch_num)) + 
  facet_wrap(~variable,ncol=1, scales="free")+
    geom_blank(aes(y = y_min)) +
    geom_blank(aes(y = y_max))+
  geom_line(aes(color=`Batch_num`)) + 
  geom_point(aes(color = `Batch_num`)) + 
    geom_hline(data = pcb_unspike_hline, aes(yintercept = `mean`, linetype= `linetype_mean`), color = "blue", size=0.6)+
  ...
  theme(axis.title.y = element_text(size=10, face="bold", colour = "black"),
        ...
        strip.text = element_text(face = "bold", size=10))+
  labs(y="Concentration", x = "Batch number")+
  ggtitle("QC Unspiked : PCB distribution")+
  annotate("text",x=10, y=0.00, size=4, label = paste("n=",max_min_unspike_pcb$`n (per variable)`[1] ))
...
OCPPBDE_stats_IQR <- notes_OCPPBDE %>% 
  select(c('HCB Results','Oxychlordane Results','trans-Nonachlor Results', 'pp-DDE Results', 'pp-DDT Results')) %>% 
    replace(is.na(.), 0) %>% 
    rename_with(~ str_remove(., " Results"), everything())
...
PCB_stats_IQR <- notes_PCB %>% 
  select(c('PCB-118  Results', 'PCB-153 Results','PCB-138 Results','PCB-156 Results', 'PCB-157 Results', 'PCB-180 Results', 'PCB-170 Results')) %>% 
    replace(is.na(.), 0) %>% 
    rename_with(~ str_remove(., " Results"), everything())
