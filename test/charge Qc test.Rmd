...
#Filter by samples that are matrix spike
OCPPBDE_QC_Spiked<-OCPPBDE %>% 
  group_by(Type) %>% 
...
#Spiked QC recovery was calculated with target of 0.5ppm
target_recovery_ocp = (~ ./0.5*(100))
...
PCB_QC_Spiked$Level <- PCB_QC_Spiked$Level %>% replace(is.na(.), 0)
...
OCPPBDE_QC_Spiked %>%
  kbl(
    caption = "<left><span style='font-size:30px'>OCPS: Spiked QCs </span></left>") %>% 
  kable_styling( stripe_color = "gray!6",
    bootstrap_options = c("striped", "hover")) %>% 
...
avg_plot1<- max_min_spiked_ocp %>%
  mutate(value=max_min_spiked_ocp$value * 0.5) %>%
